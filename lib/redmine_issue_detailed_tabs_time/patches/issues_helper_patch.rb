...
module RedmineIssueDetailedTabsTime
  module Patches
    module IssuesHelperPatch
      ...
      def self.included(base)
        base.class_eval do
          ...
          def get_issue_history_tab_entries_with(changesets)
            ...
            entries = @issue.journals + @issue.time_entries + changesets
            entries.sort! do |x,y|
              x.send(sort_methdod_for(x)) <=> y.send(sort_methdod_for(y))
            end
            entries.reverse! if User.current.wants_comments_in_reverse_order?
            entries
          end

          def sort_methdod_for(entry)
            entry.respond_to?(:committed_on) ? :committed_on : :created_on
          end

          def get_issue_history_tabs(entries, journals, changesets)
            tabs = []
          ...
          def draw_entry_in_tab(entry, index, journals)
            ...
            elsif entry.is_a?(Changeset)
              draw_changeset_in_tab(entry, index)
            else
              ""
            end
            c
...
unless IssuesHelper.included_modules.include? RedmineIssueDetailedTabsTime::Patches::IssuesHelperPatch
  IssuesHelper.send :include, RedmineIssueDetailedTabsTime::Patches::IssuesHelperPatch
end
